...
if __name__ == "__main__":
    ...
    # model.eval()
    # image_paths = [os.path.join(pathDataset+"/data", file) for file in os.listdir(pathDataset+"/data") if file.endswith(('.jpg', '.png', '.jpeg'))]
    # selected_index = [5,int(pose_list.shape[2]/2),int(pose_list.shape[2]-1)]
    # for index in selected_index:
    #     print("starting img")
    #     original_image = np.asarray(Image.open(image_paths[index]))
    #     image, _, f_px = depth_pro.load_rgb(image_paths[index])
    ...
    #     plotter.project_points_Twc(depth, original_image, pose_list[:,:,index])

    #         # Create a new Matplotlib figure for depth map visualization
    #     plt.figure(figsize=(8, 6))
    #     plt.imshow(depth, cmap="plasma")  # Use 'plasma' for better depth visualization
    #     plt.colorbar(label="Depth (meters)")
    #     plt.title(f"Depth Map - Image Index {index}")
    #     plt.axis("off")
